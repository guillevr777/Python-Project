...
from db.schemas.Persona import user_schema, users_schema

from bson import ObjectId

router = APIRouter(prefix="/Persona_db", tags=["Persona_db"])
...
async def users():
    ...
    # de la base de datos
    return users_schema(db_client.ejemplo.Personas.find())
...
async def add_user(user: User):
    ...
    id= db_client.ejemplo.Personas.insert_one(user_dict).inserted_id
...
async def modify_user(id: str, new_user: User):
    ...
    try:
        ...
        db_client.ejemplo.Personas.find_one_and_replace({"_id":ObjectId(id)}, user_dict)
...
async def delete_user(id:str):
    found = db_client.ejemplo.Personas.find_one_and_delete({"_id":ObjectId(id)})
...
def search_user_id(id: str):    
    ...
    try:
        ...
        user = user_schema(db_client.ejemplo.Personas.find_one({"_id":ObjectId(id)}))
...
def search_user(name: str, surname: str):
    ...
    try:
        # Si algo va mal en la búsqueda dentro de la base de datos se lanzará una excepción,
        # así que la controlamos
        user = user_schema(db_client.ejemplo.Personas.find_one({"name":name, "surname":surname}))
        return User(**user)
    except:
        return {"error": "User not found"}
